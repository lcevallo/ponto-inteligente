import { Lancamento, Tipo, LancamentoService } from './../../../shared';
...
export class AtualizacaoComponent implements OnInit {
  ...
  obterDadoLancamento() {
    this.lancamentoService.buscarPorId(this.lancamentoId).subscribe(
      ...
      err => {
        let msg: string = 'Error obteniendo lanzamiento';
  ...
  atualizar() {
    ...
    this.lancamentoService.atualizar(this.obterLancamentos(dados)).subscribe(
      data => {
        const msg: string = 'Lanzamiento actualizado con exito!';
        this.snackBar.open(msg, 'Exito', { duration: 5000 });
        this.router.navigate(['/admin']);
      },
      err => {
        let msg: string = 'Intente en instantes nuevamente.';
        if (err.status == 400) {
  ...
  obterLancamentos(dados: any): Lancamento {
